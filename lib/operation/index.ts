  ...
  export class File {
    ...
    constructor({ logFilePath, opts }: Arguments.FileTransport) {
      ...
      this.autoFlushInterval =
        opts?.autoFlushInterval || Default.ConfigFiles.autoFlushInterval;
      this.logBuffer = new Buffers(opts?.bufferSize || 1024);
      this.rotation = new Rotation(opts?.maxSize || Infinity);
    ...
    protected shouldFlush(messageSize: number): boolean {
      return (
        this.logBuffer.isBufferFull() ||
        this.fileStream.getSize() + messageSize > this.maxSize
      );
    }
